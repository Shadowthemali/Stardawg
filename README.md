...
- Premium dark and gold aesthetic.
- Static GitHub Pages hosting with no backend required.
- Dedicated whitepaper page for deeper project information.
- Live chart section for tracking $DAWG price, volume, and market trends.
- Trading links to major Solana ecosystem tools.
...
- `whitepaper.html` â€” whitepaper page.
- `version.txt` â€” current deploy hash.
- `assets/` â€” project images and other static assets.

## Local development

To preview the site locally, open `index.html` in a browser or serve the folder with a local static server.

## Contributing

Contributions are welcome. Please keep updates consistent with the siteâ€™s existing tone and branding, use relative paths for internal links, and avoid committing macOS metadata files such as `.DS_Store`.